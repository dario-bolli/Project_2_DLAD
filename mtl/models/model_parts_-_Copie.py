...
class DecoderDeeplabV3p(torch.nn.Module):
    ...
    def forward(self, features_bottleneck, features_skip_4x):
        ...
        features_ASPP = F.interpolate(
            features_skip_4x, size=features_skip_4x.shape[2:], mode='bilinear', align_corners=False
        )
        #1x1 conv2d on lowest feature
        features_lowest = self.features_to_concatenation(features_4x)

...
class ModelDistillation(torch.nn.Module):
    def __init__(self, cfg, outputs_desc):
        ...
        #Self-Attention
        self.attention_semseg = SelfAttention(ch_out-1, 1)
        self.attention_depth = SelfAttention(1, 1)

        #Decoder 3 and 4
        self.decoder3 = Decoder(1,ch_out-1, ch_out-1)
        self.decoder4 = Decoder(1,1,1)
    ...
    def forward(self, x):
        ...
        #Self-Attention
        features_attention_semseg = self.attention_semseg(predictions_1x_semseg)
        features_attention_depth = self.attention_depth(predictions_1x_depth)

        #Decoder 3 and 4
        out_semseg = self.decoder3(features_attention_depth, predictions_1x_semseg)
        out_depth = self.decoder4(features_attention_semseg, predictions_1x_depth)
        ...
        for task, num_ch in self.outputs_desc.items():
            if task == 'semseg':
                out[task] = out_semseg[:, offset:offset+num_ch, :, :]
                offset += num_ch
            elif task == 'depth':
                out[task] = out_depth[:,:, :, :]
                offset += num_ch

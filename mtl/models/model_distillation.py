...
class ModelDistillation(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        predictions_4x_semseg,features_dec1 = self.decoder_semseg(features_task_semseg, features[4])
        ...
        for task, num_ch in self.outputs_desc.items():
            if task == 'semseg':
                out[task] = [out_semseg_1x[:, offset:offset+num_ch, :, :],predictions_1x_semseg[:, offset:offset+num_ch, :, :]]
                offset += num_ch
            elif task == 'depth':
                out[task] = [out_depth_1x[:,:, :, :],predictions_1x_depth[:,:, :, :]]
                offset += num_ch
